...
pub fn spatial_map_typed(
    ...
    // all-NaN success-shaped result after every pixel fails. This subsumes
    // both the historical #458 B3 counts+LM+energy-scale instability guard
    // and the CountsWithNuisance+LM hoist: every count variant plus LM is
    // rejected at the boundary now.
    //
    // Issue #634: `fit_energy_scale` + `fit_temperature` is supported —
    ...
    // (Auto resolves to PoissonKL on counts input; counts + LM was
    // rejected at preflight).  When false (a transmission LM input),
    // per-pixel SpectrumFitResult.deviance_per_dof is `None`, so the
    // spatial deviance_per_dof_map should also be `None` — otherwise
    // GUI / Python consumers using `is_some()` to label GOF as "D/dof"
    ...
    // kernel reaches past the data, else the data grid) so each per-pixel
    // `PrecomputedTransmissionModel` applies Beer-Lambert + resolution on
    // the working grid and extracts the data points last — matching
    // `forward_model`.  `xs` (data-grid σ) is still needed for shape
    // validation; `work_xs` carries the working-grid σ.  When the layout is
    // the identity `work_xs` is left unset and the model uses the data-grid
    // σ directly.
    ...
    // evaluate or broaden σ.  When the layout is the identity the working
    // grid IS the data grid, so no working-grid σ is needed and we must NOT
    // pay for the full per-isotope `broadened_cross_sections_on_working_grid`
    // just to discover the layout is trivial.
    ...
    let (xs, work_xs) = match config.precomputed_cross_sections().cloned() {
        // Caller supplied data-grid σ.  When the grid is extended the model
        // needs working-grid σ, so recompute it from resonance data (the
        // data-grid σ alone cannot be de-extracted back onto the working
        // grid).  When the grid is not extended the supplied σ is already the
        // working-grid σ and Doppler broadening is skipped entirely.
    ...
    // reusing the layout computed above.  Only attached when the grid is
    // extended so the per-pixel precomputed model extracts data points after
    // resolution; `None` when the working grid is the data grid.
    ...
    let fast_config = if config.fit_temperature() {
        ...
        let mut cfg = cfg
            .with_precomputed_cross_sections(xs)
            .with_compute_covariance(true);
        // Issue #608: attach the working-grid σ + layout whenever the grid
        // was extended, so each per-pixel `PrecomputedTransmissionModel`
        // applies resolution on the working grid and extracts the data
        // points last.  `with_precomputed_cross_sections` (above) clears any
        // stale work σ, so this must come AFTER it.
...
mod tests {
    ...
    #[test]
    fn test_spatial_map_typed_counts_lm_is_rejected() {
        let data = u238_single_resonance();
        let energies: Vec<f64> = (0..51).map(|i| 1.0 + (i as f64) * 0.1).collect();
        let (t_3d, _) = synthetic_4x4_transmission(&data, 0.0005, &energies);
        let n_e = energies.len();
        ...
        let err = spatial_map_typed(&input, &config, None, None, None)
            .expect_err("counts + LM must be rejected at the spatial boundary");
        let msg = err.to_string();
        assert!(
            msg.contains("counts") && msg.contains("least-squares") && msg.contains("Poisson"),
            "error must explain the count-domain engine requirement, got: {msg}"
    ...
    /// `(Counts + LM + fit_energy_scale=true)` stays rejected — historically
    /// as the #458 B3 instability guard, now subsumed by the blanket
    /// counts+LM route rejection.
    #[test]
    ...
    fn test_spatial_map_transmission_poisson_rejected_up_front() {
        ...
        // Fractional transmission is not Poisson count data; the per-pixel
        // rejection would otherwise silently produce an all-NaN map.
        let config = UnifiedFitConfig::new(
            energies,
            vec![rd],
            vec!["U-238".into()],
        ...
        let err = spatial_map_typed(&data, &config, None, None, None)
            .expect_err("transmission + Poisson-KL must be rejected up-front");
        let msg = err.to_string();
        assert!(
            matches!(err, PipelineError::InvalidParameter(_)),
            "expected InvalidParameter, got {err:?}"
