...
class TestExactResolvedCountsRoute:
    ...
    quadrature the route derives from the detector-time edges, then fitted
...
class TestExactCountArgumentGuards:
    ...
    def test_nodes_per_bin_alone_rejected(self, u238_data):
        kwargs = self._base_kwargs(u238_data)
        with pytest.raises(ValueError, match="nodes_per_bin requires"):
            nereids.fit_counts_spectrum_typed(nodes_per_bin=1, **kwargs)

    def test_nodes_per_bin_is_required_with_the_exact_inputs(self, u238_data):
        kwargs = self._base_kwargs(u238_data)
        with pytest.raises(ValueError, match="nodes_per_bin is required"):
            nereids.fit_counts_spectrum_typed(
    ...
    def test_zero_nodes_per_bin_rejected(self, u238_data):
        kwargs = self._base_kwargs(u238_data)
