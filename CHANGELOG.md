...
- **rand 0.9→0.10 and egui 0.33→0.34 ecosystem upgrades** (#670, #671):
  ...
  - `rand`/`rand_chacha` → 0.10, `rand_distr` → 0.6 (single `rand_core`
    0.10 in the active build graph). No API changes at our one call site
    (the `ic_closed_loop` seeded-noise test): the seeded ChaCha12 stream is
    algorithm-stable, so closed-loop temperature recovery passes with no
    re-baseline or tolerance change.
