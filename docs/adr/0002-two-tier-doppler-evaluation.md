...
- Tier 1, continuous: a resolved SLBW or MLBW source with `√E > 8u` (`u = √(k_B·T/A)`) whose thermal support window `[(√E−8u)², (√E+8u)²]` lies inside its resolved range, with no other evaluable range overlapping the window and no File-3 term, integrates the kernel over the resonance equation at error-controlled quadrature (`continuous_doppler`: Gauss–Kronrod G10/K21 panels split at resonance breakpoints and at the knots of an energy-dependent scattering radius AP(E), refined by global bisection, value and temperature derivative converged together, hard limits as errors).
  Neither tier alters the sign of a cross-section: an SLBW total that goes negative where two same-J interference terms outweigh the shared potential term is integrated and convolved as it is.
...
- Gate temperature: a fixed-temperature evaluation gates at that temperature; a free-temperature fit gates once at the fit's upper bound (`TEMPERATURE_FIT_UPPER_BOUND_K`, 5000 K).
  ...
  A plan remembers a fingerprint of the grid and of the source it was decided on and refuses any other input, so a plan can never be applied to a same-length shifted grid or a same-count different source.
...
- Tier 1 costs about five times the sampled table for one broadening of Hf-177 on the VENUS grid; free-temperature fits pay it per probe, mitigated by deciding the route once per fit, converging the temperature derivative on the same panels as the value so the Jacobian does no second integration, sharing one plan across spatial pixels, and hoisting the Doppler + Beer–Lambert product out of the resolution-calibration objective when the energy scale is pinned.
