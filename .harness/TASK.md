...
- [x] R23 (process): "Commits GPG-signed, atomic" — check: git log --show-signature on the branch commits. Evidence (branch commits to date): `git log --show-signature 19d0ea2..HEAD` -> 5 commits, every one `gpg: Good signature from "Chen Zhang (kedokudo)"`; atomic (contract / lints / CI-pins / contract-updates split). Re-verify as later commits land.
